...
public class EndermanLookForPlayerGoal_v26_2_R1 extends NearestAttackableTargetGoal<Player> {
    ...
    public EndermanLookForPlayerGoal_v26_2_R1(EnderMan enderman, @Nullable TargetingConditions.Selector pathfindertargetcondition_a) {
        super(enderman, Player.class, 10, false, false, pathfindertargetcondition_a);
        this.enderman = enderman;
        this.isAngerInducing = (entityliving, worldserver) -> (v26_2_R1.isLookingAtMe(enderman, (Player) entityliving) || enderman.isAngryAt(entityliving, worldserver)) && !enderman.hasIndirectPassenger(entityliving);
    ...
    public boolean canContinueToUse() {
        if (pendingTarget != null) {
            if (v26_2_R1.isLookingAtMe(enderman, pendingTarget)) {
    ...
    public void tick() {
        ...
        if (pendingTarget != null) {
            ...
            if (target != null && !enderman.isPassenger()) {
                if (v26_2_R1.isLookingAtMe(enderman, (Player) target)) {
                    if (target.distanceToSqr(enderman) < 16.0) {
                        enderman.teleport();
                    }
                    teleportTime = 0;
                }
                else if (target.distanceToSqr(enderman) > 256.0 && teleportTime++ >= adjustedTickDelay(30) && v26_2_R1.teleportTowards(enderman, target)) {
                    teleportTime = 0;
                }
            }
            super.tick();
