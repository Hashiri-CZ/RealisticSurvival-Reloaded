...
public class RealisticSeasons extends CompatiblePlugin {
    ...
    public RealisticSeasons(HLPlugin plugin) {
        super(plugin, NAME);
    }

    @Override
    public boolean otherLoadOptions() {
        return me.casperge.realisticseasons.RealisticSeasons.getInstance().getTemperatureManager().getTempData().isEnabled() && HLModule.getModule(TanModule.NAME).isGloballyEnabled();
    }
