...
public class DragonUtils {
    ...
    public static void performMeleeAttack(EnderDragon dragon, LivingEntity defender) {
        if (HLMob.isMob(dragon)) {
            switch (HLMob.getMob(dragon)) {
    ...
    public static void performSpecialFireAbility(EnderDragon dragon, LivingEntity defender) {
        if (!BurnTask.hasTask(defender.getUniqueId())) {
            new BurnTask(HLPlugin.getPlugin(), defender, (CONFIG.getInt("Dragon.FireDragon.InfernoAbility.Stage" + getStage(dragon))), CONFIG.getInt("Dragon.FireDragon.InfernoAbility.TickPeriod")).start();
        }
    }
    ...
    public static void performSpecialIceAbility(EnderDragon dragon, LivingEntity defender) {
        if (!FreezeTask.hasTask(defender.getUniqueId())) {
            new FreezeTask(HLPlugin.getPlugin(), getStage(dragon), defender).start();
        }
    }
    ...
    public static void performSpecialLightningAbility(EnderDragon dragon, LivingEntity defender) {
        ...
        if (!ElectrocuteTask.hasTask(defender.getUniqueId())) {
            new ElectrocuteTask(HLPlugin.getPlugin(), getStage(dragon), defender).start();
        }
    }
    ...
    public static Collection<ItemStack> generateLoot(EnderDragon dragon) {
        ...
        ItemStack heart = HLItem.getItem("dragon_heart_" + lowercaseBreed);
        ItemStack skull = HLItem.getItem("dragon_skull_" + lowercaseBreed);
        ItemStack scales = HLItem.getItem("dragonscale_" + variant.toString().toLowerCase());
        ItemStack blood = HLItem.getItem("dragon_blood_" + lowercaseBreed);
        ItemStack flesh = HLItem.getItem("dragon_flesh_" + lowercaseBreed);
    ...
    public static boolean isDragon(Entity entity) {
        if (HLMob.isMob(entity)) {
            return switch (HLMob.getMob(entity)) {
