...
public abstract class GenericBreath extends BukkitRunnable {
    ...
    protected final HLPlugin plugin;
    protected final FileConfiguration config;
    protected int ticks = 0;

    public GenericBreath(EnderDragon dragon, Location target, HLPlugin plugin, BreathType type) {
        this.dragon = dragon;
    ...
    public GenericBreath(Dragon dragon, Location target, HLPlugin plugin, BreathType type) {
    ...
    public void run() {
        ...
        if (ticks > 10) {
            Predicate<Entity> filter = entity -> !(entity.getUniqueId().equals(dragon.getUniqueId()) || (HLMob.isMob(entity) && HLMob.getMob(entity).equals(HLMob.getMob(dragon))));
