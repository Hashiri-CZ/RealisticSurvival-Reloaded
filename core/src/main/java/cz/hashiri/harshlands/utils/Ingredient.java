...
public class Ingredient {
    ...
    private final List<HLItem> items = new ArrayList<>();
            ...
            if (raw.matches("^\\(([a-zA-Z_,]+)\\)$")) {
                ...
                for (String s : entries) {
                    ...
                    if (type == null) {
                        ...
                        switch (type) {
                            case ITEM -> items.add(HLItem.getItem(s));
                ...
                if (type == null) {
                    ...
                    switch (type) {
                        case ITEM -> items.add(HLItem.getItem(raw));
    ...
    public static boolean isValid(@Nullable String raw) {
        ...
        if (raw.matches("^\\(([a-zA-Z_,]+)\\)$")) {
            ...
            for (String s : entries) {
                ...
                else if (!(Utils.isInEnum(s, Material.class) || HLItem.isHLItem(s))) {
                    return false;
                }
            }
            return true;
            ...
            else return Utils.isInEnum(raw, Material.class) || HLItem.isHLItem(raw);
        }
    }

    public List<HLItem> getItems() {
        return items;
    }
            ...
            for (Object obj : col) {
                if (obj instanceof Material mat) {
                    ...
                    if (HLItem.isHLItem(item)) {
                        this.items.add(HLItem.getItem(HLItem.getNameFromItem(item)));
                    }
                }
            }
        }
    ...
    public final boolean test(@Nullable ItemStack item) {
        ...
        if (HLItem.isHLItem(item)) {
            for (HLItem i : items) {
                if (Utils.isItemReal(i) && Objects.equals(HLItem.getNameFromItem(item), i.getName())) {
                    return true;
                }
            }
            return false;
