...
public class AuraSkills extends CompatiblePlugin {
    ...
    public AuraSkills(@Nonnull RSVPlugin plugin) {
        super(plugin, NAME);
        ...
        if (isIntegrated) {
            ...
            Bukkit.getPluginManager().registerEvents(new AuraSkillsRequirementsListener(plugin), plugin);
        }
    }
    ...
    private void initializeReflection(@Nonnull RSVPlugin plugin) {
        try {
